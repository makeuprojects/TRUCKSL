...
export default function ChoferProfileCard({
  ...
  return (
    <motion.div
      layoutId={layoutId}
      className="bg-[#1E293B]/95 backdrop-blur-xl border border-[#233554]/50 rounded-[32px] p-6 text-slate-100 shadow-2xl relative max-w-xl w-full h-[85vh] flex flex-col focus:outline-none"
    ...
    >
      ...
      {/* Header */}
      <div className="flex items-center justify-between border-b border-[#112240] pb-4 shrink-0">
        <div className="flex items-center space-x-3.5">
          <div className="w-12 h-12 rounded-2xl bg-blue-500/10 border border-blue-300/20 flex items-center justify-center text-[#1E3A8A]">
            <User className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-lg font-extrabold text-slate-900 leading-tight font-sans">
        ...
        <button
          onClick={onClose}
          className="text-slate-300 hover:text-slate-900 p-2 bg-[#0A192F] border border-[#112240] rounded-xl transition"
      ...
      <div className="flex-1 overflow-y-auto pr-1 py-5 space-y-6 scrollbar-thin scrollbar-thumb-slate-800">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {/* Presupuesto */}
          <div className="bg-slate-950/60 border border-[#112240] p-4 rounded-2xl">
          ...
          {/* Total Ejecutado */}
          {/* Saldo Restante Card */}
          <div className="bg-[#112240] border border-[#233554] p-4 rounded-2xl col-span-2 sm:col-span-1">
            ...
            <div className="flex items-baseline space-x-1 mt-1.5 font-sans">
              ...
              <span className="text-xs font-mono font-bold text-slate-400 font-sans"> / {budgetNum.toLocaleString('es-BO', { minimumFractionDigits: 0 })} Bs</span>
            </div>
        ...
        {/* Spent Progress Bar */}
        <div className="bg-[#112240] border border-[#233554] p-4 rounded-2xl space-y-2">
          <div className="flex justify-between items-center text-[10px] font-mono font-bold">
            <span className="text-slate-400 uppercase tracking-wider">Porcentaje Consumido</span>
            <span className={percentSpent > 85 ? 'text-rose-400' : 'text-blue-400'}>
          ...
          <div className="h-2 w-full bg-[#0A192F] rounded-full overflow-hidden">
            <div
              className={`h-full transition-all duration-500 ease-out rounded-full ${
                percentSpent > 85 ? 'bg-rose-500' : percentSpent > 50 ? 'bg-orange-500' : 'bg-blue-500'
              }`}
        ...
        <div className="bg-[#0A192F]/40 p-4 rounded-2xl border border-[#112240] flex items-center justify-between gap-3 flex-wrap">
          <div className="space-y-0.5">
            <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wide">Acciones de Operaciones</h4>
            <p className="text-[10px] text-slate-300">Acciones reservadas para Don Saúl.</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3.5 py-2 text-xs bg-[#112240] hover:bg-slate-750 text-slate-800 border border-[#233554]/60 rounded-xl transition font-extrabold flex items-center gap-1.5 cursor-pointer"
            >
        ...
        <div className="flex items-center gap-2.5 px-1 text-[10px] text-slate-300 font-mono">
          ...
          <span>PIN de Acceso Conductor: <strong className="text-slate-900 bg-slate-950 px-2 py-0.5 rounded border border-[#112240]">{chofer.pin_acceso || '0000'}</strong></span>
        ...
        <div className="space-y-3.5">
          <div className="flex items-center justify-between border-b border-[#112240] pb-2">
            <h3 className="text-xs font-black uppercase text-slate-700 flex items-center gap-2 tracking-tight">
              <History className="w-4 h-4 text-[#1E3A8A]" />
              Bitácora de Compras de la Semana ({driverExpenses.length})
            </h3>
            <span className="text-[10px] font-mono text-slate-600">Don Saúl Audits</span>
          </div>

          {driverExpenses.length === 0 ? (
            <div className="p-6 text-center bg-slate-950/30 border border-slate-850 rounded-2xl space-y-2">
              <AlertCircle className="w-7 h-7 text-slate-700 mx-auto" />
              <p className="text-xs text-slate-600 uppercase tracking-widest font-bold">Sin Compras</p>
              <p className="text-[10px] text-slate-600">Este conductor aún no ha ingresado recibos de ruta.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {driverExpenses.map((gasto, index) => {
                ...
                return (
                  <div key={`${gasto.id_gasto}-${index}`} className="bg-slate-950/60 border border-slate-850 p-4 rounded-2xl space-y-3 hover:border-[#233554] transition">
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-extrabold text-slate-900">{gasto.tipo_gasto}</span>
                          <span className="text-[9px] bg-[#0A192F] border border-[#112240] text-slate-300 font-mono px-2 py-0.5 rounded">
                            {gasto.fecha}
                          </span>
                        </div>
                      ...
                      <div className="text-right shrink-0">
                        <span className="text-xs text-slate-600 font-mono block">Monto</span>
                      ...
                      <div className="space-y-1.5 pt-1 border-t border-slate-900">
                        ...
                        <div className="flex flex-wrap gap-2.5">
                          {images.map((img, i) => (
                            <div 
                              key={i} 
                              className="relative w-14 h-14 rounded-xl overflow-hidden border border-[#112240] bg-slate-950 cursor-pointer group shrink-0"
        ...
        {selectedImage && (
          <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4">
            <motion.div 
              ...
              className="relative max-w-lg w-full bg-[#0A192F] border border-[#112240] rounded-3xl p-3 overflow-hidden shadow-2xl"
            >
              <div className="flex justify-between items-center px-3 py-1.5 border-b border-[#112240]">
                <span className="text-xs font-bold text-slate-300">Verificador de Factura Digital</span>
                <button 
                  onClick={() => setSelectedImage(null)}
                  className="text-slate-300 hover:text-slate-900 p-1 bg-slate-950 border border-slate-850 rounded-lg"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
